    ...
    <View className="flex-1 bg-gray-50 dark:bg-black">
      ...
      <ScrollView className="flex-1 p-4">
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-lg font-bold text-gray-800 dark:text-white">Eventos em {selectedDate}</Text>
          {isLeader && (
        ...
        {isLoading ? <ActivityIndicator /> : (
          eventsForDate.length === 0 ? (
            <Text className="text-gray-500 text-center py-4 dark:text-gray-400">Nenhum evento neste dia.</Text>
          ) : (
            eventsForDate.map(event => (
              ...
              >
                <View className="flex-1">
                  <Text className="font-bold text-gray-800 dark:text-white">{event.title}</Text>
                  <Text className="text-xs text-gray-500 dark:text-gray-400">{event.type === 'service' ? 'Culto' : 'Ensaio'} • {new Date(event.event_date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                </View>
                {isLeader && (
